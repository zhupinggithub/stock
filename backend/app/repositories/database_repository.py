...
def import_prediction(candidate_path: Path, factor_path: Path, summary_path: Path, db: Engine | None = None) -> int:
    ...
    with db.begin() as conn:
        ...
        conn.execute(text("""INSERT INTO prediction_run(model_code,model_version,base_date,top_n,backtest_trade_days,backtest_sample_count,backtest_up_rate,backtest_avg_return,market_avg_return,excess_return)
          VALUES('multi_factor_rank','1.0.0',:base,:top,:days,:samples,:up,:avg,:market,:excess)
          ON DUPLICATE KEY UPDATE top_n=VALUES(top_n),backtest_trade_days=VALUES(backtest_trade_days),backtest_sample_count=VALUES(backtest_sample_count),backtest_up_rate=VALUES(backtest_up_rate),backtest_avg_return=VALUES(backtest_avg_return),market_avg_return=VALUES(market_avg_return),excess_return=VALUES(excess_return)"""),{"base":base,"top":len(candidates),"days":summary.get("样本内检验交易日"),"samples":summary.get("候选样本数"),"up":summary.get("候选次日上涨比例"),"avg":summary.get("候选平均次日收益"),"market":summary.get("全市场平均次日收益"),"excess":summary.get("候选相对收益")})
        run_id=conn.execute(text("SELECT id FROM prediction_run WHERE model_code='multi_factor_rank' AND model_version='1.0.0' AND base_date=:base"),{"base":base}).scalar_one()
...
def import_intraday(detail_path: Path, group_path: Path, summary_path: Path, db: Engine | None = None) -> int:
    ...
    with db.begin() as conn:
        run=conn.execute(text("SELECT id FROM prediction_run WHERE base_date=:base ORDER BY id DESC LIMIT 1"),{"base":base}).scalar_one()
        ...
        for _,r in detail.iterrows():
            ...
            conn.execute(text("INSERT INTO intraday_candidate(intraday_run_id,candidate_id,current_price,previous_close,current_return,market_excess,is_up) VALUES(:ir,:cid,:price,:prev,:ret,:excess,:up)"),{"ir":intraday_id,"cid":cid,"price":clean(r.get("盘中最新价")),"prev":clean(r.get("昨收")),"ret":clean(r.get("预测期内盘中收益")),"excess":clean(r.get("当前相对全市场")),"up":None if pd.isna(r.get("当前是否上涨")) else int(bool(r.get("当前是否上涨")))})
...
def import_verification(detail_path: Path, group_path: Path, summary_path: Path, db: Engine | None = None) -> int:
    ...
    with db.begin() as conn:
        run=conn.execute(text("SELECT id FROM prediction_run WHERE base_date=:base ORDER BY id DESC LIMIT 1"),{"base":s["预测基准日"]}).scalar_one()
