...
onMounted(()=>Promise.all([refresh(),can('schedule:view')?loadSchedule():Promise.resolve()]));onUnmounted(()=>timer.value&&clearInterval(timer.value));
</script>
<template>
  ...
  <div class="section"><div class="section-heading"><h2>最近任务</h2><span>再次点击同一任务 ID 可收起</span></div><DataTable :columns="columns" :rows="jobs"/></div>
