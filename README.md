...
预测因子、动态权重、概率、连续入选与两个模型验证口径详见 [预测模型计算逻辑说明](PREDICTION_MODEL_LOGIC.md)。
...
登录会话默认最长有效 60 天，连续 30 天没有访问后台接口则失效，可在 `.env` 中通过 `STOCK_SESSION_DAYS` 和 `STOCK_SESSION_IDLE_DAYS` 调整。修改后需重启服务并重新登录，浏览器才会取得新的 Cookie 有效期。
